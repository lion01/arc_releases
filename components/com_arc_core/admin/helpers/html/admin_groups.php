 ...
 * @since		1.5
 */
class JHTMLAdmin_Groups
{
	/**
	 * Generates the link which holds the information on the children of the given node
	 */
	function nodelink( $id = false, $base = false )
	{
		if( $base === false ) {
			$base = 'index.php?option=com_arc_core&view=ajax&task=loadtreenode&format=xml';
		}
		
		return $base.( ($id !== false) ? '&node='.urlencode($id) : '' );
	}
	 ...
	 */
	function grouplist( $parent, $onlyCurrent = true )
	{
		$u = &ApotheosisLib::getUser();
		$db = &JFactory::getDBO();
		$date = date('Y-m-d H:i:s');
		$tableName = '#__apoth__tmp_admin_groups_'.time();
		
		// get all the children of the given node
		// ... first figure out all the courses that we want to consider
		$query = 'CREATE TABLE '.$tableName.' ENGINE=MyISAM AS'
		...
		if( $onlyCurrent ) {
			// limit query to current groups only
	...
	function grouptree( $id, $multiple = true, $defaults = false, $partGroups = false )
	{
		...
		$dataUrl = JHTML::_('admin_groups.nodelink');
		...
		if( $multiple ) {
			...
			$opt['onExpand'] = "\\function(node, state) { if(state) { this.setDefault(node); } }";
		}
		...
		<div id="<?php echo $id; ?>_div_outer" class="arc_moo_tree_control">
			...
			<div class="buttons">
				<input type="button" id="<?php echo $id; ?>_unselect" value="Unselect All" />
