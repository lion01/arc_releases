 ...
 * @since 0.1
 */
class ApothReportPl extends ApothReport
{
	/**
	...
	function init( $student, $group )
	{
		...
		$this->_fields['set']         = new ApothFieldHidden ( $this, 'set',         'group',    0,   0,   0,   0, 0.25, 0, 0.5, 0.5,   '0%', '1.5em', '',               $this->_data->group, '');
		$this->_fields['setname']     = new ApothFieldWord   ( $this, 'setname',     '',        36, 126,  63, 138, 0.25, 0, 0.5, 0.5,  '30%', '1.5em', 'Teaching Set:',  $this->_data2->group_name, '');
		$this->_fields['attitude']    = new ApothFieldLookup ( $this, 'attitude',    '',        63, 126,  90, 138, 1, 1, 0.5, 0.5,  '30%', '1.5em', 'Attitude:',      $student, $group );
		$this->_fields['current_txt'] = new ApothFieldText   ( $this, 'current_txt', 'text_1',  36, 126, 198, 180, 1, 1, 12.5, 0.5, '100%',   '9em', 'Current Performance:',     $this->_data->text_1, 'Please write your main comments here');
...
//		$this->_fields['current_txt']->titleMarginTop = 12; //glitterfully
		
		$banked = array('current_txt', 'targets', 'method');
		$lists = array('attitude');
		foreach($banked as $b) {
