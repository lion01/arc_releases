 ...
 * @since 		1.5
 */
class plgUserArc_User extends JPlugin
{
	/**
	 * Constructor
	 *
	 ...
	 * @since 1.5
	 */
	function plgUserArc_User(& $subject, $config) {
	...
	function onLoginUser($user, $options = array())
	{
		...
		// Let's determine if we are autoupdating user details
		$autoupdate = isset($options['autoupdate']) ? $options['autoupdate'] : $this->params->get('autoupdate', 1);
		
		// If we are autoupdating check for changes from the authentication source and update
		if($autoupdate && !$instance->get( 'tmp_user', false )) {
			$changes = false;
			if($instance->get('name')           != $user['fullname'])       {$instance->set('name',           $user['fullname']); $changes = true;}
			if($instance->get('username')       != $user['username'])       {$instance->set('username',       $user['username']); $changes = true;}
			if($instance->get('password_clear') != $user['password_clear']) {$instance->set('password_clear', $user['password_clear']); $changes = true;}
			if($instance->get('email')          != $user['email'])          {$instance->set('email',          $user['email']); $changes = true;} // Result should contain an email (check)
			
			// If there are changes save them
			if($changes && !$instance->save()) {
