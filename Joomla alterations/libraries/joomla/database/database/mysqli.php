...
class JDatabaseMySQLi extends JDatabase
{
	...
	*/
	function __construct( $options )
	{
		...
		$ssl		= array_key_exists( 'dbssl', $options )		? $options['dbssl']		: false;
		$sslcert	= array_key_exists( 'dbsslcert', $options )	? $options['dbsslcert']	: '';
		$sslkey		= array_key_exists( 'dbsslkey', $options )	? $options['dbsslkey']	: '';
		$sslca		= array_key_exists( 'dbsslca', $options )	? $options['dbsslca']	: '';
		// *** end Arc mod
		...
		// *** Arc modification to enable ssl
		if( $ssl ) {
			$this->_resource = @mysqli_init();
			@mysqli_ssl_set( $this->_resource, $sslkey, $sslcert, $sslca, NULL, NULL );
			@mysqli_real_connect( $this->_resource, $host, $user, $password, NULL, $port, $socket );
		}
		else {
			$this->_resource = @mysqli_connect($host, $user, $password, NULL, $port, $socket);
		}
