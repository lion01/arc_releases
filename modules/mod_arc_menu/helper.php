 ...
 * @since		1.5
 */
class modApothMenuHelper
{
	function buildXML($params)
	{
		$menu = new JApothMenuTree($params);
		$items = &JSite::getMenu();
		...
		if ( is_array($rows)) {
			while (count($rows) && !is_null($row = array_shift($rows)))
			{
				if (array_key_exists($row->parent, $ids)) {
					...
					// *** This (and the "require" for the library) is the only difference for an Arc menu.
					// Just a small modification to make a check on user's permissions before adding a menu item
					$requirements = $row->query;
					$requirements['Itemid'] = $row->id;
					$actionId = ApotheosisLib::getActionId($requirements);
					if( !is_null($actionId) && ApotheosisLibAcl::getUserPermitted( $user->id, $actionId ) ) {
//					if( ($actionId == false) || ApotheosisLibAcl::getUserPermitted( $user->id, $actionId ) ) {
						$menu->addNode($params, $row);
						// record loaded parents
						$ids[$row->id] = true;
					}
					// *** End Arc modification
	...
	function &getXML($type, &$params, $decorator)
	{
		...
		if (!isset($xmls[$type])) {
			$cache =& JFactory::getCache('mod_arc_menu');
			$string = $cache->call(array('modApothMenuHelper', 'buildXML'), $params);
			$xmls[$type] = $string;
		}
	...
	function render(&$params, $callback)
	{
		switch ( $params->get( 'menu_style', 'list' ) )
		{
			...
			default :
				...
				$xml = modApothMenuHelper::getXML($params->get('menutype'), $params, $callback);
				if ($xml) {
 ...
 * @since		1.5
 */
class JApothMenuTree extends JTree
{
	/**
	...
	{
		$this->_params		=& $params;
		$this->_root		= new JApothMenuNode(0, 'ROOT');
	...
	function addNode(&$params, $item)
	{
		...
		$node = new JApothMenuNode($item->id, $item->name, $item->access, $data);
 ...
 * @since		1.5
 */
class JApothMenuNode extends JNode
{
	/**
	 * Node Title
	 */
